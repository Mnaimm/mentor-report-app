...
export default function MentorDashboard() {
  ...
  const handleViewDetails = (mentee) => {
    ...
    const progress = `${mentee.reportsThisRound} daripada ${mentee.expectedReportsThisRound}`;
    alert(`Maklumat Usahawan:\n\nNama: ${mentee.name}\nSyarikat: ${mentee.businessName}\nProgram: ${mentee.program}\nKohort: ${mentee.batch}\nEmel: ${mentee.email}\nTelefon: ${mentee.phone || 'Tiada'}\n\nPusingan Semasa: ${mentee.currentRound}\nKemajuan: ${progress}\nStatus: ${mentee.status}`);
  ...
  const groupedMentees = () => {
    ...
    filteredMentees.forEach(mentee => {
      const batch = mentee.batch || 'Kohort Tidak Diketahui';
      if (!groups[batch]) {
        groups[batch] = [];
      }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">Memuatkan papan pemuka...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          <p className="text-red-600 font-semibold mb-2">Ralat Memuatkan Papan Pemuka</p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Papan Pemuka Mentor</h1>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-6 mt-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span><span className="font-semibold text-red-600">{dashboardData.stats?.overdue || 0}</span> Lewat</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-lg">🟡</span>
                <span><span className="font-semibold text-yellow-600">{dashboardData.stats?.dueSoon || 0}</span> Perlu Tindakan Segera</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-green-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Mengikut Jadual</p>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-yellow-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Tamat Tempoh Terdekat</p>
                ...
                <>
                  {overdue.length > 0 && (
                    <div className="border-l-4 border-red-500 pl-4">
                      <p className="text-sm font-semibold text-red-600 mb-1">⚠️ Lewat ({overdue.length})</p>
                      <p className="text-sm text-gray-600">
                        {overdue.slice(0, 3).map(m => m.name).join(', ')}
                        {overdue.length > 3 && ` +${overdue.length - 3} lagi`}
                      </p>
                    </div>
                  )}
                  ...
                  {thisWeek.length > 0 && (
                    <div className="border-l-4 border-yellow-500 pl-4">
                      <p className="text-sm font-semibold text-yellow-600 mb-1">Minggu Ini ({thisWeek.length})</p>
                      <p className="text-sm text-gray-600">
                        {thisWeek.slice(0, 3).map(m => m.name).join(', ')}
                        {thisWeek.length > 3 && ` +${thisWeek.length - 3} lagi`}
                      </p>
                    </div>
                  )}
                  ...
                  {nextWeek.length > 0 && (
                    <div className="border-l-4 border-blue-500 pl-4">
                      <p className="text-sm font-semibold text-blue-600 mb-1">Minggu Depan ({nextWeek.length})</p>
                      <p className="text-sm text-gray-600">
                        {nextWeek.slice(0, 3).map(m => m.name).join(', ')}
                        {nextWeek.length > 3 && ` +${nextWeek.length - 3} lagi`}
                      </p>
                    </div>
                  )}
                  ...
                  {later.length > 0 && (
                    <div className="border-l-4 border-gray-400 pl-4">
                      <p className="text-sm font-semibold text-gray-600 mb-1">Akan Datang ({later.length})</p>
                      <p className="text-sm text-gray-600">
                        {later.slice(0, 3).map(m => m.name).join(', ')}
                        {later.length > 3 && ` +${later.length - 3} lagi`}
                      </p>
                    </div>
                  )}

                  {overdue.length === 0 && thisWeek.length === 0 && nextWeek.length === 0 && later.length === 0 && (
                    <p className="text-sm text-gray-500 italic">🎉 Tahniah! Semua usahawan mengikut jadual.</p>
                  )}
                </>
              );
            })()}
      ...
      <div className="bg-white rounded-lg shadow-md p-4 md:p-6 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4">
          ...
          <div className="sm:col-span-2 lg:col-span-2">
            ...
            <input
              type="text"
              placeholder="Cari nama, nama syarikat, atau emel..."
          ...
          <div>
            ...
            >
              ...
              <option value="due_soon">Tamat Tempoh Terdekat</option>
              <option value="overdue">Lewat</option>
              <option value="mia">MIA (Tidak Dapat Dihubungi)</option>
              <option value="pending_first_session">Belum Bermula</option>
            </select>
          </div>

          {/* Batch Filter */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Kohort</label>
            <select
              value={filters.batch}
        ...
        <div className="mt-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm font-medium text-gray-700">Susun ikut:</span>
            <button
              onClick={() => setSortBy('urgency')}
              className={`px-3 py-1.5 text-sm rounded-lg font-medium transition-colors ${sortBy === 'urgency'
            ...
            <button
              ...
              className={`px-3 py-1.5 text-sm rounded-lg font-medium transition-colors ${sortBy === 'dueDate'
                ? 'bg-blue-600 text-white shadow-sm'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 text-sm rounded-lg font-medium transition-colors ${sortBy === 'batch'
                ? 'bg-blue-600 text-white shadow-sm'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 text-sm rounded-lg font-medium transition-colors ${sortBy === 'name'
                ? 'bg-blue-600 text-white shadow-sm'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 text-sm rounded-lg font-medium transition-colors ${sortBy === 'progress'
                ? 'bg-blue-600 text-white shadow-sm'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
            >
          ...
          <p className="text-sm text-gray-600 whitespace-nowrap">
            Menunjukkan {filteredMentees.length} daripada {dashboardData?.mentees?.length || 0} usahawan
          </p>
        </div>
      </div>
      ...
      {filteredMentees.length === 0 ? (
        <div className="bg-white rounded-lg shadow-md p-12 text-center">
          ...
          <p className="text-xl font-semibold text-gray-700 mb-2">Tiada usahawan ditemui</p>
          <p className="text-gray-500">
            {filters.status !== 'all' || filters.batch !== 'all' || filters.program !== 'all' || filters.search
              ? 'Sila ubah tetapan carian anda'
              : 'Anda belum mempunyai usahawan seliaan.'}
          </p>
        </div>
      ) : (
                ...
                <div className="sticky top-0 z-10 bg-gradient-to-r from-gray-50 to-gray-100 border-l-4 border-blue-500 px-4 py-3 mb-4 rounded-lg shadow-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="bg-blue-600 text-white px-3 py-1 rounded-full text-sm font-semibold">
                      {mentees.length} {mentees.length === 1 ? 'usahawan' : 'usahawan'}
                    </span>
                  </div>
                </div>
              )}
