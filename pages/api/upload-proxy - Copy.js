...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (reportType === 'maju-um') {
      ...
      url = process.env.APPS_SCRIPT_LAPORAN_MAJU_UM_URL;
      if (!url) return res.status(500).json({ error: 'APPS_SCRIPT_LAPORAN_MAJU_UM_URL not set' });
      ...
      url = process.env.APPS_SCRIPT_LAPORAN_MAJU_URL;
      if (!url) return res.status(500).json({ error: 'APPS_SCRIPT_LAPORAN_MAJU_URL not set' });
      ...
      url = process.env.APPS_SCRIPT_BANGKIT_URL;
      if (!url) return res.status(500).json({ error: 'APPS_SCRIPT_BANGKIT_URL not set' });
    ...
    const makeHttpsRequest = (url, body) => {
      return new Promise((resolve, reject) => {
        ...
        const options = {
          ...
          timeout: 60000, // 60 seconds - allows large uploads on slower connections
          followRedirect: true, // Follow 302 redirects automatically
        ...
        const req = https.request(options, (res) => {
          ...
          if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
            console.log(`🔄 Following redirect (${res.statusCode}) to:`, res.headers.location);

            // Follow the redirect recursively
            makeHttpsRequest(res.headers.location, body)
              .then(resolve)
              .catch(reject);
            return;
          }
          ...
          res.on('end', async () => {
            try {
              ...
              if (encoding === 'gzip') {
                console.log('🗜️ Decompressing gzip response...');
              ...
              console.error('❌ Decompression error:', decompressError.message);
    ...
    try {
      ...
      // Check if response is HTML (auth page, error page, etc.)
      if (text.trim().startsWith('<')) {
        return res.status(502).json({
          error: 'Apps Script returned HTML instead of JSON',
          message: 'This usually indicates an authentication or deployment issue. Please verify the Apps Script deployment URL is correct and publicly accessible.'
        });
      }

      // Unknown parse error - return generic error with truncated response
      return res.status(502).json({
        error: 'Invalid response from Apps Script',
        message: 'The Apps Script endpoint returned a response that could not be parsed as JSON.',
